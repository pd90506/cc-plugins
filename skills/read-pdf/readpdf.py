#!/usr/bin/env python3
"""Render any PDF's pages and regions to PNG so a vision model can read them.

Document-neutral: works on invoices, forms, slides, manuals, contracts, reports,
books, and scanned pages alike. Uses PyMuPDF when present and installs it on
first run if missing. Falls back to pypdf + macOS `sips` if that install fails,
so rendering still works offline.

Commands:
  probe <pdf>                    pages, size, text-layer presence, any captions
...
`pages` renders the whole document for a page-by-page read; `-o figures` saves
crops (from `find`) or pages next to your notes. Default output is a temp dir.
"""
import argparse
import os
import re
...
def main():
    ...
    if a.cmd == "probe":
        if fitz:
            ...
            sample = "".join(doc[p].get_text() for p in range(min(3, doc.page_count)))
            if len(sample.strip()) < 20:
                print("text layer: none (scanned/image-only - vision-read the pages)")
            else:
                print("text layer: present (born-digital - `text` for a fast dump)")
    ...
    if a.cmd == "text":
        # Born-digital fast path: dump the text layer, page-delimited. An empty
        # layer means a scanned/image-only PDF -> render and vision-read instead.
        if fitz:
            doc = fitz.open(a.pdf)
            npages = doc.page_count
            get = lambda pno: doc[pno - 1].get_text()
        else:
            reader = _pypdf().PdfReader(a.pdf)
            npages = len(reader.pages)
            get = lambda pno: reader.pages[pno - 1].extract_text() or ""
        parts, chars = [], 0
        for pno in _parse_range(a.range, npages):
            t = get(pno)
            chars += len(t.strip())
        ...
        if a.out:
            with open(a.out, "w") as fh:
                fh.write(blob)
            print("%s  (%d chars of text)" % (a.out, chars))
        else:
            print(blob)
        if chars < 20:
            print("no text layer found - scanned/image-only PDF; render pages "
                  "and vision-read instead", file=sys.stderr)
        return
