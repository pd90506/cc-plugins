...
- **Delegation.** When the source set is broad, delegate to a subagent via the `Agent` tool (`subagent_type: general-purpose`), giving it a role (researcher / verifier) in its prompt and pointing it at the artifact file to write. Do not assume named bundled agents exist — describe the role in the task itself. For a small set, do the work directly.
