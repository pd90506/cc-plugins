...
- **Delegation.** When the audit is non-trivial, delegate to a subagent via the `Agent` tool (`subagent_type: general-purpose`), giving it a role (researcher / verifier) in its prompt and pointing it at the artifact file to write. Do not assume named bundled agents exist — describe the role in the task itself. Otherwise do the audit directly.
...
- Delegate to a subagent via the `Agent` tool (`subagent_type: general-purpose`) with a researcher role for evidence gathering and to another `Agent` call with a verifier role to verify sources and add inline citations when the audit is non-trivial.
