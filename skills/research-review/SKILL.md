...
- **Delegation.** When the artifact is large, delegate to a subagent via the `Agent` tool (`subagent_type: general-purpose`), giving it a role (researcher / reviewer) in its prompt and pointing it at the artifact file to write. Do not assume named bundled agents exist — describe the role in the task itself. Otherwise do the review directly.
...
6. Delegate to a subagent via the `Agent` tool (`subagent_type: general-purpose`) with a researcher role and/or a reviewer role only when the artifact is large enough to benefit from delegation. Otherwise do the lead-owned review directly. Never merely say a subagent was spawned; either delegate for real or continue yourself.
